     ...
     $scope.crearOrden = function (){
      ...
      $scope.Ordenes = [];
          ...
          /*$scope.columnDefs1= [
                               ...
                               $scope.columnDefs= [
                               {field:'id', displayName: 'Id orden',visible: true , width : '6%' ,pinnedLeft:true },
                               {field:'estadoOrden', displayName: 'Estado orden',visible: true , width : '12%' },
                               {field:'tipoServicioNombre', displayName: 'Tipo servicio',visible: true , width : '18%' },
                               {field:'clienteCodigo', displayName: 'Cliente',visible: true , width : '10%' },
                               {field:'numeroDocumentoOrdenCliente', displayName: 'Numero documento',visible: true , width : '15%' },
                               {field:'numeroDocumentoConsolidadoCliente', displayName: 'Consolidado',visible: true , width : '10%' },
                               {field:'destinatarioNombre', displayName: 'Destinatario',visible: true , width : '35%' },
                               {field:'destinatarioNumeroIdentificacion', displayName: 'Nit',visible: true , width : '10%' },
                               {field:'destinoCiudadNombre', displayName: 'Ciudad',visible: true , width : '10%' },
                               {field:'destinoDireccion', displayName: 'Direccion',visible: true , width : '35%' },
                               {field:'usuarioActualizacion', displayName: 'Usuario',visible: true , width : '18%' },
                               {field:'fechaActualizacion', displayName: 'Fecha actualizacion',visible: true , width : '15%' }
                               ]                                  
       ...
       /* console.log('http://'+ $scope.serverData.ip+':'+ $scope.serverData.puerto+'/satelite/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario?id_tipo_servicio=1&estadoOrden=NO_CONFIRMADA&id_usuario='+$scope.usuario.id);
          ...
          //$http.get('http://'+ $scope.serverData.ip+':'+ $scope.serverData.puerto+'/satelite/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario?id_tipo_servicio=1&estadoOrden=NO_CONFIRMADA&id_usuario=2')
              ...
              if(window.localStorage.getItem("estadoOrdenCache")  === null){                 
                console.log("no existe chache entra a ESTADO ORDEN");
       ...
       $rootScope.cargarOrdenes = function (){
        window.localStorage.setItem('estadoOrdenCache',$scope.jsonListaOrdenes.estadoOrden);
         ...
         /*if ($scope.jsonListaOrdenes.idCliente != undefined ){
          ...
          console.log("oirtr mas ");

          if($scope.jsonListaOrdenes.idCliente != undefined ){
            $scope.cadena ='http://'+ $scope.serverData.ip+':'+ $scope.serverData.puerto+'/'+contexto+'/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario-con-paginacion?id_tipo_servicio='+$scope.jsonListaOrdenes.idServicio+'&id_estado_orden='+$scope.jsonListaOrdenes.estadoOrden+'&id_usuario='+$scope.usuario.id+'&id_cliente='+$scope.jsonListaOrdenes.idCliente+'&pageNumber='+ paginationOptions.pageNumber+'&pageSize='+ paginationOptions.pageSize; 
          }else{             
            $scope.cadena ='http://'+ $scope.serverData.ip+':'+ $scope.serverData.puerto+'/'+contexto+'/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario-con-paginacion?id_tipo_servicio='+$scope.jsonListaOrdenes.idServicio+'&id_estado_orden='+$scope.jsonListaOrdenes.estadoOrden+'&id_usuario='+$scope.usuario.id+'&pageNumber='+ paginationOptions.pageNumber+'&pageSize='+ paginationOptions.pageSize ;   
          }
         // $scope.cadena ='http://'+ $scope.serverData.ip+':'+ $scope.serverData.puerto+'/'+contexto+'/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario-con-paginacion?id_tipo_servicio=3&id_estado_orden=CONFIRMADA&id_usuario=1020&pageNumber='+paginationOptions.pageNumber+'&pageSize='+paginationOptions.pageSize;           
         // $scope.cadena ="http://192.168.10.15:8080/sateliteDev/ordenes/ordenes-x-tipo_servicio-x-estado-x-usuario-con-paginacion?id_tipo_servicio=3&id_estado_orden="+$scope.jsonListaOrdenes.estadoOrden+"&id_usuario=1020&pageNumber="+paginationOptions.pageNumber+"&pageSize="+paginationOptions.pageSize;           
          ...
          .then(function(response){                  
           $scope.respuesta= response.data;
           console.log("json cargado todas las ordenes ===> " );
           console.log($scope.respuesta.content) ; 
           $scope.datatableData = [] ;    
           for (var i = 0; i < $scope.respuesta.content.length; i++) {              
                       /*$scope.datatableData =  $scope.datatableData.concat([{
                                     ...
                                     $scope.datatableData =  $scope.datatableData.concat([{
                                       id :$scope.respuesta.content[i].id,
                                       estadoOrden : $scope.respuesta.content[i].estadoOrden , 
                                       tipoServicioNombre: $scope.respuesta.content[i].tipoServicioNombre,
                                       clienteCodigo: $scope.respuesta.content[i].clienteCodigo,
                                       numeroDocumentoOrdenCliente : $scope.respuesta.content[i].numeroDocumentoOrdenCliente,
                                       numeroDocumentoConsolidadoCliente: $scope.respuesta.content[i].numeroDocumentoConsolidadoCliente,
                                       destinatarioNombre: $scope.respuesta.content[i].destinatarioNombre,
                                       destinatarioNumeroIdentificacion : $scope.respuesta.content[i].destinatarioNumeroIdentificacion,
                                       destinoCiudadNombre : $scope.respuesta.content[i].destinoCiudadNombre,
                                       destinoDireccion : $scope.respuesta.content[i].destinoDireccion,
                                       usuarioActualizacion : $scope.respuesta.content[i].usuarioActualizacion,
                                       fechaActualizacion : $scope.respuesta.content[i].fechaActualizacion
                                     }]);
                                   }                 
        ...
        $scope.showConfirmacion = function(ev) {        
          ...
          $mdDialog.show({
            controller: DialogControllerNuevoProducto,
        ...
        function DialogControllerNuevoProducto($scope ,$rootScope , $mdDialog ,serverData,estados ,gridApi , login ) 
        {
